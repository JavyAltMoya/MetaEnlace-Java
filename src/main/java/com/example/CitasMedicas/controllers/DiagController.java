...
public class DiagController {
    ...
    public DiagModel guardarDiag(@RequestBody DiagModel diag){
        return this.diagService.guardarDiag(diag);
    }

    // Se usa para obtener el id de un diagnóstico
    @GetMapping(path= "/{id}")
    public Optional<DiagModel> getDiagByID(@PathVariable Long id){
    ...
    @PutMapping(path= "/{id}")
    public DiagModel updateDiagByID(@RequestBody DiagModel request,@PathVariable("id") Long id){
